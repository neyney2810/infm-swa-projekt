...
export async function GET(request: Request) {
  try {
    ...
    if (!land || !bundeslandMap[land]) {
      return NextResponse.json({ error: "Invalid or missing Bundesland code" }, { status: 400 });
    }
    ...
    const filteredData = parsedData.data.filter((row: any) => {
      const matchesBundesland = row.Bundesland && row.Bundesland.includes(bundeslandName);
